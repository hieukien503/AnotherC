...
for path in ['./test/', './main/AnotherC/parser/', './main/AnotherC/utils/', './main/AnotherC/astgen/', './main/AnotherC/checker/', './main/AnotherC/codegen/']:
    sys.path.append(path)
ANTLR_JAR = os.environ.get('ANTLR_JAR')
TARGET_DIR = '../target'
GENERATE_DIR = 'main/AnotherC/parser'
...
def main(argv):
    if len(argv) < 1:
        ...
        subprocess.run(["java", "-jar", ANTLR_JAR, "-o", "../target",
                       "-no-listener", "-visitor", "main/AnotherC/parser/AnotherC.g4"])
    elif argv[0] == 'clean':
