...
class DoubleLit(Expr):
    ...
    def __str__(self):
        return "DoubleLit({})".format(str(self.val))
...
class CharLit(Expr):
    ...
    def __str__(self):
        return "CharLit({})".format(self.val)
...
class MemberAccess(Expr):
    def __init__(self, firstExpr: Expr | str, secondExpr: Expr):
        self.firstExpr = firstExpr
        self.secondExpr = secondExpr
    
    def __str__(self):
        return "MemberAccess({}, {})".format(str(self.firstExpr), str(self.secondExpr))


class NewExpr(Expr):
    def __init__(self, structName: str, param: List[Expr]):
        self.structName = structName
        self.param = param

    def __str__(self):
        return "NewExpr(" + str(self.structName) + ",[" + ','.join(str(i) for i in self.param) + "])"
...
class IfStmt(Stmt):
    def __init__(self, cond: Expr, tstmt: Stmt, estmt: List[ElseIfStmt] or None = None, fstmt: Stmt or None = None):
        self.cond = cond
        self.tstmt = tstmt
        self.estmt = estmt
        self.fstmt = fstmt
...
class ForStmt(Stmt):
    def __init__(self, init: VarDecl, cond: Expr, upd: Expr, stmt: Stmt):
        self.init = init
        self.cond = cond
        self.upd = upd
        self.stmt = stmt
...
class ParamDecl(Decl):
    def __init__(self, name: str, typ: Type, initExpr: Expr or None = None):
        self.name = name
        self.typ = typ
        self.initExpr = initExpr

    def __str__(self):
        return "Param({}, {}{})".format(self.name, str(self.typ), ", " + str(self.initExpr) if self.initExpr else "")


class FuncDecl(Decl):
    def __init__(self, name: str, return_type: Type or None = None, params: List[ParamDecl] = None, body: BlockStmt or None = None):
        self.name = name
    ...
    def __str__(self):
        ...
        return "ConstructorDecl({}, [{}]{})".format(self.name, ", ".join([str(param) for param in self.params]), ", " + str(self.body) if self.body else "")


class StructDecl(Decl):
    def __init__(self, name: str, varDecls: List[VarDecl], constructors: List[FuncDecl]):
        self.name = name
        self.varDecls = varDecls
        self.constructors = constructors
    
    def __str__(self):
        return "StructDecl({}, AttributeDecl([{}]), Constructor([{}]))".format(self.name, ", ".join([str(varDecl) for varDecl in self.varDecls]), ", ".join([str(constructor) for constructor in self.constructors]))
